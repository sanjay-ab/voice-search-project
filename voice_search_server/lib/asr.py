...
def create_asr(model_path, return_full_dictionary=False):
    xlsr_model = load_xlsr_model(model_path)
    asr_model = load_asr_model(model_path)

    return ASR(xlsr_model, asr_model, return_full_dictionary)
...
class ASR:
    ...
    def __init__(self, xlsr_model, asr_model, return_full_dictionary=False):
        self.xlsr_model = xlsr_model
        self.asr_model = asr_model
        self.return_full_dictionary = return_full_dictionary
    ...
    def decode(self, feats):
        if self.return_full_dictionary:
            return self.asr_model.decode(Matrix(feats))
